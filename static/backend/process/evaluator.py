...
def main(dataset_name):
    ...
    session_dir = os.path.join(static_dir, f"dataset/{dataset_name}/coauthor-json")
    json_path = os.path.join(static_dir, "import_dataset")
    topic_dir = os.path.join(json_path, f"{dataset_name}.csv")
    ...
    if not os.path.exists(eval_dir):
        ...
        print(f"Create foler eval_results in {eval_dir}")
    else:
        print(f"Folder eval_results already exist in {eval_dir}.")
    ...
    for file_name in os.listdir(session_dir):
        if file_name.endswith(".jsonl"):
            ...
            if not session.empty:
                ...
                write_file(eval_dir, session_id, result)